...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h4 class="text-default-900 text-2xl font-medium mb-2">Product Management</h4>
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <a href="{{ route('products.create') }}" class="btn bg-primary border-primary text-white hover:bg-primary-600">
                        ...
                        <table class="w-full text-sm text-left text-default-500">
                            ...
                            <tbody class="divide-y divide-default-200">
                                ...
                                <tr class="bg-white hover:bg-default-50 transition-all">
                                    ...
                                    <td class="px-6 py-4">
                                        <code class="bg-default-100 px-2 py-1 rounded text-xs">{{ $product['sku'] }}</code>
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ $product['category'] }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            @if($product['status'] == 'Out of Stock') bg-red-100 text-red-800
                                            @elseif($product['status'] == 'Low Stock') bg-yellow-100 text-yellow-800
                                            @else bg-green-100 text-green-800
                                            @endif">
                                            {{ $product['status'] }}
                                        </span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            console.log('Product ID:', itemId, 'Product Name:', itemName); // Debug log
